...
// This method just returns the 16-byte data returned from the Pixart sensor - it's the quickest and most economical way of 
// getting the data back.
byte* PVision::quickread()
{
    //IR sensor read
    Wire.beginTransmission(IRslaveAddress);
    Wire.send(0x36);
...
// This version of read populates the blob attributes of the class, it's the most user friendly version, but slower than quickread
byte PVision::read()
{
    //IR sensor read
    Wire.beginTransmission(IRslaveAddress);
